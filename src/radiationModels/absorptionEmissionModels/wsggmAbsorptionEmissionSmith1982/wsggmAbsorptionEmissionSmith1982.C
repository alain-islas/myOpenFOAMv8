...
namespace Foam
{
	...
	{
        	defineTypeNameAndDebug(wsggmAbsorptionEmissionSmith1982, 0);
...
Foam::radiationModels::absorptionEmissionModels::wsggmAbsorptionEmissionSmith1982::wsggmAbsorptionEmissionSmith1982
(
...
Foam::radiationModels::absorptionEmissionModels::wsggmAbsorptionEmissionSmith1982::aCont(const label bandI) const
{
    ...
    forAll(a, celli)
    {
	    ...
	    scalar XkC02 = mixture.Y(indexCO2)[celli]/(mixture.Wi(indexCO2)*invWt);
	    scalar XkH20 = mixture.Y(indexH2O)[celli]/(mixture.Wi(indexH2O)*invWt);
	    ...
	    forAll(emissivityCoeffs_,i)
        	{
		        ...
		        for(label j=0; j<=fittingFactors_.size(); j++) //start from j=0 and end j=fittingFactors_.size()
		        {
		            weightingFactor += fittingFactors_[i][j]*pow(limitedDimlessTemperature, (j)); //exponent j
		        }
...
Foam::tmp<Foam::volScalarField>
Foam::radiationModels::absorptionEmissionModels::wsggmAbsorptionEmissionSmith1982::eCont(const label bandI) const
{
    return aCont(bandI);
}
